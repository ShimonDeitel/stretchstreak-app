...
final class Store: ObservableObject {
    ...
    private let fileName = "stretchstreak_entries.json"
    ...
    static func seedData() -> [Entry] {
        let now = Date()
        return [
            Entry(title: "Sample Check-in One", detail: "First example entry.", date: now),
            Entry(title: "Sample Check-in Two", detail: "Second example entry.", date: now.addingTimeInterval(-86400)),
            Entry(title: "Sample Check-in Three", detail: "Third example entry.", date: now.addingTimeInterval(-172800)),
        ]
    }
}
